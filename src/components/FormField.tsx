...
const FormField = ({ field }: FormFieldProps) => {
    ...
    switch (field.type) {
      ...
      case 'combo-box':
        return (
          <div className="flex flex-col">
            <label className="mb-1 text-sm font-medium text-gray-700">{field.label}</label>
            <select className="px-3 py-1.5 border border-gray-200 rounded-md text-sm" onClick={e => e.stopPropagation()}>
              ...
              {field.options?.map(option => (
                <div key={option.id} className="flex items-center">
                  <input
                    ...
                    onClick={e => e.stopPropagation()}
                  />
                  <label htmlFor={option.id} className="text-sm">{option.value}</label>
                </div>
              ))}
            </div>
          </div>
              ...
              {field.options ? field.options.map(option => (
                ...
                <div className="flex items-center">
                  <input
                    ...
                    onClick={e => e.stopPropagation()}
                  />
                  <label htmlFor={field.id} className="text-sm">Option</label>
                </div>
              )}
            </div>
          </div>
  ...
  return (
    <div
      className={`relative p-2 border rounded-md flex ${isSelected ? 'border-blue-400 bg-blue-50' : 'border-gray-200'}`}
    ...
    >
      ...
      <div className="flex items-start ml-2">
        <button className="p-1 text-gray-400 hover:text-gray-600" title="Move">
